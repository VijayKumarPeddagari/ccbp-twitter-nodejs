...
// api 7
const convertLikedUserNameDBObjectToResponse0bject = (dbObject) => {
  return {
    Likes: dbObject,
  };
};
...
app.get(
  ...
  async (request, response) => {
    ...
    if (getTweetIds.includes(parseInt(tweetId))) {
      ...
      const getUsernameReplyTweetsQuery = `select user.name, reply.reply from user inner join reply on user.user_id=reply.reply
    where reply.tweet_id=${tweetId};`;
      const getUsernameReplyTweets = await database.all(
        getUsernameReplyTweetsQuery
      );
      //console.log(getUsernameReply Tweets);
      /*console.log(
...
app.get("/user/tweets", authenticationToken, async (request, response) => {
  ...
  const getTweetsDetailsQuery = `
    ...
    from
        ...
        inner join like on like.tweet_id=tweet.tweet_id inner join reply on
...
app.post("/user/tweets/", authenticationToken, async (request, response) => {
  ...
  const postRequestQuery = `insert into tweet (tweet, user_id) values ("${tweet}", ${getUserId.user_id});`;
